...
from config import llama3_8b, llama3_13b, llama3_34b
from rope import get_rope_params, apply_rope

class RMSNorm(nn.Module):
  def __init__(self, config = llama3_8b, eps = 1e-7):
    super().__init__()
    self.eps = eps
...
class FeedForward(nn.Module):
  def __init__(self, config = llama3_8b):
    super().__init__()
...
if __name__ == "__main__":
  model = Llama3(llama3_8b)
  
  total_params = sum(p.numel() for p in model.parameters())
  print(f"Total number of parameters: {total_params:,}")  # 8,030,261,248 --> 8B parameters